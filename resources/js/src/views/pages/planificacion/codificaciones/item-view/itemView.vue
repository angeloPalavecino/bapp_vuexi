...
<template>
  <div id="page-item-view">
    ...
    <vs-alert color="danger" title="Rut no encontrado" :active.sync="item_not_found">
      <span>El rut con id: {{ $route.params.itemId }} no fue encontrada. </span>
      <span>
        <span>Mira </span><router-link :to="{name:'codificaciones'}" class="text-inherit underline">Todas los rut</router-link>
      </span>
    </vs-alert>
    ...
    <div id="item-data" v-if="item_data">
      ...
      <!-- MAPA -->
     <google-map-basic :lat="item_data.lat" :lng="item_data.lng" :data_patrones="item_data_patrones"
     :latsuc="item_data.latsuc" 
     :lngsuc="item_data.lngsuc"
     :sucursal="item_data.sucursal"
     ></google-map-basic>
    <!-- MAPA -->
         ...
         <!-- Buttons -->
          <div class="vx-col w-full flex mt-4" id="account-manage-buttons">
            <vx-tooltip color="primary" text="Editar">
              <vs-button v-if="$can('codificaciones.edit')" icon-pack="feather" icon="icon-edit" class="mr-4" :to="{name: 'codificaciones-edit', params: { itemId: $route.params.itemId }}">Editar</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Volver">
              <vs-button icon-pack="feather" icon="icon-arrow-left" class="mr-4" :to="{name: 'codificaciones'}">Volver</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Eliminar">
              <vs-button v-if="$can('codificaciones.destroy')" type="border" color="danger" icon-pack="feather" icon="icon-trash" @click="confirmDeleteRecord">Eliminar</vs-button>
            </vx-tooltip>
          </div>
           <!-- Buttons -->
  ...
  methods: {
    ...
    confirmDeleteRecord() {
      this.$vs.dialog({
        ...
        text: `Esta seguro que desea eliminar el siguiente rut "${this.item_data.rut}"`,
    ...
    deleteRecord() {
      ...
      /* UnComment below lines for enabling true flow if deleting user */
       this.$store.dispatch("itemManagement/borrarItem", {	Id: this.item_data.id, Url: this.urlApi  })
         .then(()   => { this.$router.push({name:'codificaciones'}); this.showDeleteSuccess() })
         .catch(err => { 
