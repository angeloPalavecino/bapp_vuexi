      ...
      <div class="vx-col md:w-1/2 w-full mt-2">
       ...
       <div class="vx-col md:w-1/2 w-full mt-2">
        <vs-select v-model="data_local.fuerazona" label="Fuera de Zona" name="fuerazona"  
             ...
             :danger-text="(errors.first('fuerazona') ? errors.first('fuerazona') : '')">
      ...
      <div class="vx-col w-full">
        <div class="mt-3 flex flex-wrap items-center justify-end">
          <vx-tooltip color="primary" text="Guardar">
              <vs-button v-if="$can('grupopatrones.update')" class="ml-auto mt-2" @click="save_changes" :disabled="!validateForm">Guardar Cambios</vs-button>
          </vx-tooltip>
...
export default {
  ...
  computed: {
   ...
   mounted() {
      ...
      setTimeout(() => {
                 var ids = this.data_patrones.map(x => x['id']); 
                 this.data_local.patrones = ids
