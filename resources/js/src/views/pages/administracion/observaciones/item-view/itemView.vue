...
<template>
  <div id="page-item-view">
    ...
    <vs-alert color="danger" title="Observacion no encontrado" :active.sync="item_not_found">
      <span>La observacion con id: {{ $route.params.itemId }} no fue encontrado. </span>
      <span>
        <span>Mira </span><router-link :to="{name:'observaciones'}" class="text-inherit underline">Todas las observaciones</router-link>
      </span>
    </vs-alert>
         ...
         <!-- Buttons -->
          <div class="vx-col w-full flex mt-4" id="account-manage-buttons">
            <vx-tooltip color="primary" text="Editar">
              <vs-button v-if="$can('observaciones.edit')" icon-pack="feather" icon="icon-edit" class="mr-4" :to="{name: 'observaciones-edit', params: { itemId: $route.params.itemId }}">Editar</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Volver">
              <vs-button icon-pack="feather" icon="icon-arrow-left" class="mr-4" :to="{name: 'observaciones'}">Volver</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Eliminar">
              <vs-button v-if="$can('observaciones.destroy')" type="border" color="danger" icon-pack="feather" icon="icon-trash" @click="confirmDeleteRecord">Eliminar</vs-button>
            </vx-tooltip>
          </div>
           <!-- Buttons -->
  ...
  methods: {
    confirmDeleteRecord() {
      this.$vs.dialog({
        ...
        text: `Este seguro que desea eliminar la siguiente observacion "${this.item_data.descripcion}"`,
