      ...
      <div class="vx-col w-full">
        <div class="mt-3 flex flex-wrap items-center justify-end">
          <vx-tooltip color="primary" text="Guardar">
              <vs-button v-if="$can('excepciones.update')" class="ml-auto mt-2" @click="save_changes" :disabled="!validateForm">Guardar Cambios</vs-button>
          </vx-tooltip>
  ...
  methods: {
    ...
    agregarMarker() {
          ...
          if(this.data_local.direccion){
           ...
           const lat = parseFloat(this.data_local.lat); //this.autocomplete.getPlace().geometry.location.lat();
           const lng = parseFloat(this.data_local.lng); //this.autocomplete.getPlace().geometry.location.lng();
           const direccion = this.data_local.direccion; //this.autocomplete.getPlace().formatted_address;
