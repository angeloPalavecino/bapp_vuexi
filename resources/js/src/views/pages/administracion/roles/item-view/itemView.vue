...
<template>
  <div id="page-item-view">
    ...
    <vs-alert color="danger" title="Rol no encontrado" :active.sync="item_not_found">
      <span>El rol con id: {{ $route.params.itemId }} no fue encontrado. </span>
      <span>
        <span>Mira </span><router-link :to="{name:'roles'}" class="text-inherit underline">Todos los roles</router-link>
      </span>
    </vs-alert>
    ...
    <div id="item-data" v-if="item_data">
     ...
     <!-- PERMISOS -->
    <vx-card class="mb-base" title="Permisos" collapseAction>
      ...
      <div class="block overflow-x-auto" >
        <vs-table :data="permisosOptions" pagination   max-items="20" class="permisos">
                        ...
                        <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                          ...
                          <vs-td :data="data[indextr].modulos">
                            ...
                            <vs-td v-for="(td, index) in data[indextr].roles" :key="index">
                              <vs-checkbox name="permisos" disabled="true" v-validate="'required'" icon-pack="feather" icon="icon-check" v-model="item_data.permisos" :vs-value="td.id"/>
                            </vs-td>
         ...
         <!-- Buttons -->
          <div class="vx-col w-full flex mt-4" id="account-manage-buttons">
            <vx-tooltip color="primary" text="Editar">
              <vs-button v-if="$can('roles.edit')"  icon-pack="feather" icon="icon-edit" class="mr-4" :to="{name: 'roles-edit', params: { itemId: $route.params.itemId }}">Editar</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Volver">
              <vs-button icon-pack="feather" icon="icon-arrow-left" class="mr-4" :to="{name: 'roles'}">Volver</vs-button>
            </vx-tooltip>
            <vx-tooltip color="primary" text="Eliminar">
              <vs-button v-if="$can('roles.destroy')"  type="border" color="danger" icon-pack="feather" icon="icon-trash" @click="confirmDeleteRecord">Eliminar</vs-button>
            </vx-tooltip>
          </div>
           <!-- Buttons -->
  ...
  methods: {
    confirmDeleteRecord() {
      this.$vs.dialog({
        ...
        text: `Este seguro que desea eliminar el siguiente rol "${this.item_data.name}"`,
