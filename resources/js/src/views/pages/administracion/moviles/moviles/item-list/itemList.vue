     ...
     <vx-card ref="filterCard" title="Filtros" class="items-list-filters mb-8" collapseAction refreshContentAction @refresh="resetColFilters" >
      <div class="vx-row">
        <div class="vx-col md:w-1/4 sm:w-1/2 w-full">
          <label class="text-sm opacity-75">Asociado</label>
          <v-select :options="asociadoOptions" :clearable="false" :dir="$vs.rtl ? 'rtl' : 'ltr'" v-model="asociadoFilter" class="mb-4 md:mb-0" />
        </div>
...
export default {
  ...
  data() {
    return {
      urlApi: "/car/car/",
       // Filter Options
      asociadoFilter: { label: 'Todos', value: 'all' },
      asociadoOptions: [
  ...
  methods: {
    ...
    resetColFilters() {
      ...
      this.asociadoFilter = { label: 'Todos', value: 'all' }
    ...
    traeOtrosDatos() {
      //Combo Asociados
      axios.get(`/api/v1/asociado/combo`)
        .then((res) => {
          var asociados = res.data.items;  
          asociados.push({label: 'Todos', value: 'all'})   
          this.asociadoOptions = asociados.reverse();
